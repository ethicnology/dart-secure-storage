...
class _OublietteDemoPage extends StatelessWidget {
...
class _StorageTabState extends State<_StorageTab> {
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      _setMessage('${e.code}: ${e.message ?? "unknown"}');
    } catch (e) {
      _setMessage('$e');
    } finally {
  ...
  Future<void> _load() async {
    final key = _keyController.text.trim();
    if (key.isEmpty) { _setMessage('Enter a key.'); return; }
    setState(() { _loading = true; _message = null; _fetchedValue = null; });
    try {
      ...
      child: Column(
        ...
        children: [
          ...
          TextField(
            ...
            decoration: const InputDecoration(hintText: 'e.g. api_token, refresh_token, pin', border: OutlineInputBorder()),
            textInputAction: TextInputAction.next,
          ),
          const SizedBox(height: 16),
          Text('Value (secret to store)', style: Theme.of(context).textTheme.labelLarge),
          const SizedBox(height: 4),
          TextField(
            controller: _valueController,
            decoration: const InputDecoration(hintText: 'Enter a secret string', border: OutlineInputBorder()),
            obscureText: true,
                ...
                child: Column(
                  ...
                  children: [
                    Row(
                      ...
                      children: [
                        Text('Stored value', style: Theme.of(context).textTheme.labelLarge),
                        IconButton(
...
class _BiometricTabState extends State<_BiometricTab> {
  late final Oubliette _storage;
  final _keyController = TextEditingController(text: 'biometric_secret');
  final _valueController = TextEditingController();
  String? _fetchedValue;
  String? _message;
  bool _obscureFetched = true;
  ...
  void initState() {
    super.initState();
    _storage = createOubliette(
      androidOptions: const AndroidOptions(
        keyAlias: 'biometric_key',
        authentication: AndroidAuthentication(
  ...
  Future<void> _save() async {
    ...
    try {
      await _storage.storeString(key, value);
      if (!mounted) return;
      _setMessage('Saved with biometric protection.');
    } on PlatformException catch (e) {
      _setMessage('${e.code}: ${e.message ?? "unknown"}');
    } catch (e) {
      _setMessage('$e');
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
              ...
              child: Column(
                ...
                children: [
                  Row(children: [
                    ...
                    Text('Biometric-protected storage',
                        style: Theme.of(context).textTheme.titleMedium?.copyWith(fontWeight: FontWeight.bold)),
                  ]),
                  const SizedBox(height: 8),
                  Text(
          ...
          TextField(
            controller: _keyController,
            decoration: const InputDecoration(hintText: 'e.g. seed_phrase, pin', border: OutlineInputBorder()),
            textInputAction: TextInputAction.next,
          ),
          const SizedBox(height: 16),
          Text('Value (secret to store)', style: Theme.of(context).textTheme.labelLarge),
